...
The identical mechanism runs in the editor bridge against the editor process's own log, which is where GDScript parse errors, shader compile errors, import failures, and plugin exceptions surface, and where an engine error printed by a failing edit-time call lands -- the engine prints and returns rather than raising, so without this the call reports success and its reason reaches nobody. The editor's pair is named `gd_editor_get_logs` and `gd_editor_get_errors`, mirroring `gd_screenshot` and `gd_editor_screenshot`, because the broker routes one MCP tool to one bridge; the mechanism and the cursor semantics are those above. Where the editor's log is has to be supplied rather than discovered (`CONDUIT_LOG_FILE`, section 15 and `docs/api-gaps.md`). An agent editing scripts gets compile-time diagnostics without ever launching the game (`gd_script_validate` in section 8 builds on this).
...
The state shown is link-level by design. The bridge observes accept, close, and the heartbeat of section 7.5 on its own listener, never the broker's identity or the MCP client's session — the hello frame flows bridge-to-broker only. Accept and close alone would not be enough: a socket staying open proves only that some process holds the descriptor, so a suspended broker, a descriptor inherited by a child, or a broker orphaned by the MCP client that spawned it would all read as an attached agent indefinitely. Because the broker's process lifetime tracks that client (section 6.2) *and* it answers the heartbeat, a live link is an honest proxy for an attached agent. Connection state crosses from the IO thread to the main thread as a single atomic snapshot polled once per frame, keeping section 6.4 intact.
...
The base protocol is request/response, but three things happen without being asked: a game process appears (after `gd_play`, or launched externally with the opt-in flag), a game process exits (cleanly or by crash), and the editor connection drops (editor closed or crashed). Bridges may send unsolicited `event` frames — id-less, shaped `{"event": "...", "data": {...}}` — and the broker also infers lifecycle from socket state: a game socket closing is a `game_exited` event carrying the exit reason where known. The broker exposes events two ways so both polling and push-capable clients are served: it forwards them as MCP logging notifications, and it retains a bounded ring of recent events queryable through `gd_get_events` with the same incremental-cursor shape as log tailing. Every connection carries lightweight heartbeats, id-less and shaped `{"ping": seq}` and `{"pong": seq}`, and both ends send both: whichever end sees no inbound bytes for the ping interval asks, and whichever end sees none for the liveness timeout drops the link. This is what distinguishes a peer that is gone from one that is merely quiet, in the cases a closed socket never reports — a suspended process, a descriptor a child inherited, or a broker orphaned by the MCP client that spawned it. Heartbeats are answered on the IO path at both ends, so a pong attests to the transport and deliberately not to the engine's main thread: a long export must not read as death, and main-thread responsiveness stays the business of per-request timeouts. Because the deadline arms only once a peer has answered once, a peer too old to know the frame degrades to close-only detection instead of being disconnected for speaking the protocol it was built against. The broker reconnects with backoff when the editor link drops while the editor process is still alive; a broker restart or a transient socket failure must not require restarting Godot, so the bridge listener keeps accepting new connections for its lifetime. Dropping a dead link is not cosmetic on the bridge side: a listener serves one client at a time, so a link wrongly held open locks every future broker out.
...
**The target grammar (cross-cutting).** The generic reflection tools take an optional `target` naming what they act on: a node path, or `singleton:<Class>` for an engine singleton. It is parsed in exactly one place and shared by `gd_node_call`, `gd_node_get_property`, `gd_node_set_property`, `gd_node_get_info`, `gd_scene_node_call`, `gd_scene_node_get_property`, and `gd_scene_node_set_property`, so the servers, `OS`, `Time`, `Input`, `ProjectSettings`, and `EditorInterface` are addressable without evaluation. The older `node_path` argument still works and still means a node; `target` is additive, and supplying both is an error rather than a silent preference. This is a naming mechanism, not a capability group, which is why it sits above the groups rather than inside one. Objects that are neither nodes, singletons, nor resources (`SurfaceTool`, `PhysicsDirectSpaceState3D`, `MeshDataTool`) have no stable name at all, so they are named by handle: `object:<n>` is the third scheme, resolved through a per-process table (`bridge/src/handles.rs`). `class:<Class>` is the fourth and last scheme, and the only one that resolves to no object at all: a static method has no receiver, so `FileAccess.open` and `DirAccess.open` were unreachable while the grammar named instances only. Only `gd_node_call` and `gd_scene_node_call` accept it, dispatching through `ClassDB.class_call_static`; every other target-taking tool refuses it with one shared message, because a class is not a thing to read a property from. `capture` on the result is what turns a static factory into an `object:<n>` the next call can name. A handle is taken explicitly, either by constructing a `RefCounted` class with `gd_object`/`gd_scene_object` or by passing `capture` to one of the six verbs that can hand an object back (`gd_node_call`, `gd_node_get_property`, `gd_scene_node_call`, `gd_scene_node_get_property`, `gd_resource_call`, `gd_resource_get_property`), which takes one on the top-level value that came back; it is spent as a `target`, or as `{"__type":"Object","handle":"object:<n>"}` wherever a value is accepted, so one call's result feeds another's arguments. The table is bounded and refuses at capacity rather than evicting, a handle dies with its process, and resolution goes through the instance id so an object freed underneath a handle reports `object_not_found` instead of dereferencing a dead pointer. This is the reason the bookkeeping is two tools rather than one: a handle taken out of a running game means nothing to the editor, and the tool name is where that is said. `docs/coverage-matrix.md` measures what this leaves uncovered.

**Project and session (editor bridge).** Get engine version and project metadata, list project files with filtering and pagination, read and modify project settings, manage autoloads (`gd_autoload`) and the input map (`gd_input_map`) — both are settings-file-backed writes to `project.godot`, not undo-wrapped editor state — manage export presets, set the main scene, and enable or disable editor plugins (`gd_editor_plugin`, which lists every `res://addons` directory holding a `plugin.cfg` and toggles one through `EditorInterface::set_plugin_enabled`, naming it by directory as the engine does; unlike the two settings tools above it takes effect in the running editor at once, because enabling loads the plugin). Launch, run, and stop the game (`gd_play`, `gd_stop`), which is also the mechanism that brings the game bridge online. Editor lifecycle sits here too: `gd_editor_launch` (the broker spawns `godot -e --path <project>` with a configured engine binary and waits for the bridge to connect), `gd_editor_quit`, and `gd_project_scaffold`, a broker-side tool that creates a minimal new project — a `project.godot`, the bridge addon folder, and the `.gdextension` — so an agent can start from an empty directory, the one capability that by definition cannot require an already-running editor.
...
**Scene structure (editor bridge, undo-wrapped).** Create a scene with a chosen root type, add and remove nodes (with optional initial property values on add), reparent, rename, duplicate, attach and detach scripts, read and write individual node properties on the edited scene (`gd_scene_node_get_property`, `gd_scene_node_set_property`), instantiate another scene as a child (`gd_scene_instantiate`, which sets the owner on the instance root only so the instance's internal nodes stay owned by their own scene), connect and disconnect signals (`gd_scene_signal`, `CONNECT_PERSIST` when both ends are nodes of the edited scene, live and reported as unpersisted when the source is a singleton or an object handle), manage persistent node groups (`gd_node_group`), search the edited scene by class, group, or name pattern (`gd_scene_find_nodes`), read the full scene tree as JSON, and save scenes. Every mutation goes through `EditorUndoRedoManager`.

**Scripts and resources (editor bridge).** Create a script from a template, read and write script files through the editor's file handling, create and modify `.tres` and `.res` resources through `ResourceLoader`/`ResourceSaver`, create and read shaders and themes, and manage translations (`gd_translations`, which reads and writes the four `internationalization/locale/*` settings the editor's Localization tab edits: the registered `.translation` list, the per-resource remap table, and the fallback and test locale. It is a settings-file write like `gd_autoload`, so it is not undo-wrapped. Extracting strings into a POT template is not included: that is the editor's own generator, driven from the Localization dialog, with no scripted entry point, and managing its source list would look like a capability without being one). File operations trigger a filesystem rescan; moves and renames are UID-aware per section 6.5. Script edits pair with `gd_script_validate`, which reloads the script through the engine and returns parse and compile diagnostics with line numbers (surfaced from the editor log where the API does not return them directly), so the agent gets a compile check without running the game. Shader creation gets the same log-derived compile diagnostics.

**Assets and import (editor bridge).** Read and set import settings, and export a project through a preset for CI and release builds. `gd_import_settings` reads (`op: get`) and writes (`op: set`) the `[params]` of an asset's `.import` sidecar, reimporting afterwards unless told not to. It goes through `ConfigFile` rather than the import plugin surface, because a `ResourceImporter` is one of the objects the target grammar cannot name; the only visible difference is that an importer cannot be asked for options an asset does not already carry, and in practice it writes its full default set on first import. The write is not undo-wrapped and reports `undoable: false`: an `.import` write is a file write, not edited-scene state, and putting it on the scene's history would let `gd_undo` claim to revert something the history never owned. Asset ingestion is explicit: `gd_asset_add` writes agent-supplied bytes (base64 through the broker) to a project path, triggers the import scan, and waits for the import to settle, returning the imported resource's type and UID; `gd_asset_reimport` reimports after import-setting changes. This is how textures, audio, fonts, and models produced outside Godot enter the project through the front door instead of a raw file drop the editor discovers later.
