...
**The counts below predate phase 13.** `gd_import_settings` and its rule changes
are in `scripts/coverage/`, but the tables here are from the last full run: a
regeneration also rewrites `coverage-matrix.json`, which is 8 MB and tracked with
Git LFS, so `CLAUDE.md` reserves it for when the numbers are actually wanted. The
prose sections were updated with this phase; the numbers were not. Regenerating
...
| | count |
|---|---|
| Tools registered, all flags on | 94 |
| Tools registered, default flags | 89 |
| Distinct capabilities, counting `(tool, op)` pairs | 198 |
...
denominator. 888 of 2924 in-scope headings are concepts.
...
|---|---|---|---|---|---|---|---|
| Best of both bridges | 222 (1.4%) | 11372 (73.9%) | 3796 (24.7%) | - | - | - | 15390 |
| Runtime bridge | 162 (1.1%) | 7980 (51.9%) | 7248 (47.1%) | - | - | - | 15390 |
| Editor bridge | 84 (0.5%) | 11486 (74.6%) | 3820 (24.8%) | - | - | - | 15390 |

**24.7% of the documented engine API has no semantic path.** The
...
3381 headings, 457 in excluded areas, 888 conceptual, leaving
**2036 discrete actions**, of which **570 (28.0%) are T2 or worse**.

| Docs area | T0 | T1 | T2 | T3 | T4 | T5 | actions | concepts |
|---|---|---|---|---|---|---|---|---|
| getting_started/first_2d_game | 13 | 11 | - | - | - | - | 24 | 5 |
| getting_started/first_3d_game | 9 | 24 | 3 | - | - | - | 36 | 5 |
| getting_started/step_by_step | 14 | 11 | - | - | - | - | 25 | 13 |
| tutorials/2d | 4 | 104 | 42 | 2 | - | - | 152 | 24 |
| tutorials/3d | 13 | 290 | 53 | - | - | - | 356 | 91 |
| tutorials/animation | 5 | 48 | 19 | 2 | - | - | 74 | 33 |
| tutorials/assets_pipeline | 7 | 33 | 68 | 1 | - | - | 109 | 15 |
| tutorials/audio | 22 | 26 | - | - | - | 1 | 49 | 7 |
| tutorials/editor | 17 | 14 | 1 | 35 | - | - | 67 | 23 |
...
| tutorials/rendering | 1 | 51 | 5 | - | - | - | 57 | 25 |
| tutorials/scripting | 28 | 260 | 5 | 11 | - | - | 304 | 306 |
| tutorials/shaders | - | - | 101 | - | - | - | 101 | 127 |
| tutorials/ui | 6 | 94 | 15 | 1 | - | 5 | 121 | 10 |
...
What remains is one shape of problem: 3739 members across 295
...
| T2 | 101 | 13 | no shader tool; gd_resource_create makes the resource but nothing compiles it or returns diagnostics |
| T2 | 50 | 9 | no baking tool; gd_editor_eval only |
| T2 | 47 | 13 | no tool reads or writes .import options, though whitepaper section 8 specifies one; gd_editor_eval only |
| T3 | 42 | 16 | gd_editor_ui (tier-2 control-tree driving); no semantic tool exists |
| T2 | 34 | 6 | no translation management tool; PO/POT generation is an editor menu action |
| T2 | 27 | 8 | no plugin enable/disable tool, though whitepaper section 8 specifies one |
| T2 | 23 | 4 | individual .import option, which no tool reads or writes |
| T2 | 19 | 4 | gd_animation creates value tracks only; other track types and the animation editor have no tool |
| T2 | 19 | 3 | no TileSet authoring tool; gd_tilemap paints cells but cannot define sources, terrains, or patterns |
| T3 | 18 | 7 | gd_editor_ui; the project manager is a separate editor mode with no semantic surface |
| T2 | 14 | 6 | no theme tool; gd_resource_set_property writes a .tres blind and cannot read it back |
| T2 | 13 | 4 | FileAccess and DirAccess are RefCounted helpers with no handle; only the singletons among them are reachable |
| T2 | 9 | 1 | the TileMap editor's paint tools have no semantic equivalent; gd_tilemap writes cells one at a time |
| T2 | 8 | 2 | gd_render debug_draw covers line, circle, rect, sphere, and box only; arbitrary _draw() work needs a script |
| T2 | 8 | 6 | no shader tool; shader source has no create-and-validate path |
...
| T2 | 3 | 1 | no theme tool; project-wide skinning writes theme resources blind |
