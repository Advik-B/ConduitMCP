...
**The class-reference and tutorial counts below predate phases 15, 16 and 17.** `gd_editor_plugin`,
`gd_translations`, `gd_object`, `gd_scene_object`, the target grammar on the two
signal tools, and their rule changes are all in `scripts/coverage/`, but the
tables here are from the last full run: a regeneration also rewrites
`coverage-matrix.json`, which is 8 MB and tracked with Git LFS, so `CLAUDE.md`
reserves it for when the numbers are actually wanted. The prose sections were
updated with each phase; the numbers were not. Regenerating removes this
paragraph.

Phase 16 is the large one. Regrading `object`-kind classes as T1 moves 3732
members off T2, which would take best-of-both from 3789 (24.6%) to 140 (0.9%)
and the runtime row from 7248 to 3574. (Phase 16 estimated those two by hand as
"roughly 57" and "about 3516"; replaying its rules over the committed data gives
the figures here instead.) Phase 17 takes the remainder: the 140 signals on
non-node classes (83 object, 31 singleton, 26 resource), which is every
class-reference member the two phases above left at T2 on the best-of-both
row. That row would then be **0**, the editor row 0, and
the runtime row 3460 -- every one of them a resource member, which is the
runtime-resource grading stated below rather than a gap in the surface. All of
these figures are **projected** by replaying the new rules over the committed
`coverage-matrix.json`, not measured by a regeneration, and they are stated that
way rather than quietly written into the tables.
...
This row is the exception to the note at the top of this document. It comes from
`collectToolSurface` and `capabilityIds` in `scripts/coverage/extract-tools.ts`,
which read the registered tools and need no documentation checkout at all -- so
it was measured rather than left stale, the way the class-reference tables were.
It was 96 / 91 / 200 before phases 15, 16 and 17: four tools and nineteen
capabilities, the last two of which are `gd_scene_signal`'s `emit` and `await`.
...
Nothing in the class reference. The five generic verbs the coverage matrix
named are all shipped; the best-of-both and editor rows would read 0 at T2, and
the runtime row's remaining 3460 are all resource members -- the
runtime-resource conditional described above, which is a grading rather than a
gap in the surface.
...
LFS version -- deliberately not done as part of these phases. Everything the
tables report about phases 15, 16 and 17 is projected by hand until then, and
the note at the top of this document says so.
