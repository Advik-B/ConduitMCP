...
**The counts below predate phase 15.** `gd_editor_plugin`, `gd_translations`,
and their rule changes are in `scripts/coverage/`, but the tables here are from
the last full run: a regeneration also rewrites `coverage-matrix.json`, which is
8 MB and tracked with Git LFS, so `CLAUDE.md` reserves it for when the numbers
are actually wanted. The prose sections were updated with this phase; the
numbers were not. Regenerating removes this paragraph.
