...
| `CONDUIT_GODOT_DOCS` | `scripts/coverage/*` | Root of an offline Godot HTML documentation build (the directory holding `_sources/`), used by `bun run coverage` to regenerate `docs/coverage-matrix.md`, and by `bun run coverage:check` to validate the rule tables without writing anything. `--docs <dir>` overrides it. |
