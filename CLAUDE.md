...
- `docs/coverage-matrix.json` is tracked with Git LFS (`.gitattributes`); it is 8 MB of generated audit data and the repository's whole history is smaller than that. Working on it needs `git lfs install`, or a clone yields a pointer file instead of the JSON. Nothing reads it today, so no test or script breaks without LFS, and CI checkouts do not need `lfs: true` until something does. Regenerate with `bun run coverage`, which needs an offline Godot documentation build via `CONDUIT_GODOT_DOCS`; every regeneration stores a new LFS version, so do not regenerate it as a routine step. Do not add further LFS-tracked files without asking: LFS storage and bandwidth are a finite account-level quota, not a repository setting.
