...
- Environment variables are documented in `docs/environment.md`, which is the authoritative reference. Adding or changing one means updating that file, the README flag table, and the `readme()` string literal in `scripts/pack-npm.ts` (it ships to npm), plus whitepaper section 15. Booleans go through `envFlag` (`broker/src/env.ts`, `bridge/src/env.rs`), never a bare truthiness check.
